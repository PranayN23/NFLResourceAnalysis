...
def sklearn_mlp(df, metric):
    features = df[['Value_cap_space', 'Value_draft_data', 'Previous_AV', 'Previous_PFF']]
    #features = pd.get_dummies(features)
    labels = df['Current_' + metric]
    features_train, features_test = features[:21], features[21:]
    labels_train, labels_test = labels[:21], labels[21:]
