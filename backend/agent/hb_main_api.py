...
from backend.agent.rb_agent_graph import hb_gm_agent, HB_CSV_PATH
...
app = FastAPI(title="NFL HB/RB GM Agent API")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

HB_GRADE_COL = "grades_offense"
HB_SNAP_COL  = "run_plays"
HB_PROD_STATS = ["yards", "touchdowns", "attempts"]

df_players = pd.DataFrame()
if os.path.exists(HB_CSV_PATH):
    df_players = pd.read_csv(HB_CSV_PATH)
    print(f"[HB API] Loaded {len(df_players)} rows")


@app.get("/hb-players")
async def get_hb_players():
    if df_players.empty:
...
async def team_roster(team: str = Query(...), analysis_year: int = Query(2025)):
    if df_players.empty:
        raise HTTPException(status_code=503, detail="Player database not loaded.")
    roster = get_team_roster(team, df_players, grade_col=HB_GRADE_COL, snap_col=HB_SNAP_COL, reference_year=analysis_year)
    need_score, need_label = compute_positional_need(roster, position_df=df_players, team=team,
                                                      grade_col=HB_GRADE_COL, snap_col=HB_SNAP_COL,
                                                      prod_stat_cols=HB_PROD_STATS, reference_year=analysis_year,
                                                      position_key="HB")
...
async def evaluate_player(req: EvaluationRequest):
    ...
    if req.team:
        roster = get_team_roster(req.team, df_players, grade_col=HB_GRADE_COL, snap_col=HB_SNAP_COL, reference_year=analysis_year)
        ...
        if re_signing:
            ...
            need_score, need_label = compute_positional_need(roster_without, position_df=df_players,
                team=req.team, exclude_player=req.player_name, grade_col=HB_GRADE_COL,
                snap_col=HB_SNAP_COL, prod_stat_cols=HB_PROD_STATS, reference_year=analysis_year,
                position_key="HB")
            ...
            need_score, need_label = compute_positional_need(roster, position_df=df_players,
                team=req.team, grade_col=HB_GRADE_COL, snap_col=HB_SNAP_COL, prod_stat_cols=HB_PROD_STATS, reference_year=analysis_year,
                position_key="HB")
            player_cap = 0.0
    ...
    final_state = await loop.run_in_executor(_thread_pool, hb_gm_agent.invoke, initial_state)
    if req.team:
