"""T (Offensive Tackle) GM Agent API — port 8008"""
import asyncio
...
app = FastAPI(title="NFL T GM Agent API")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

T_GRADE_COL  = "grades_pass_block"
T_SNAP_COL   = "snap_counts_offense"
T_PROD_STATS = ["snap_counts_offense", "penalties"]

df_players = pd.DataFrame()
if os.path.exists(T_CSV_PATH):
    df_players = pd.read_csv(T_CSV_PATH)
    print(f"[T API] Loaded {len(df_players)} rows")


@app.get("/t-players")
async def get_t_players():
    if df_players.empty:
