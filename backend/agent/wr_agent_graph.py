...
from backend.agent.stat_projection_utils import offense_target_load_17, inactivity_retirement_penalty, apply_inactivity_to_projection_list, apply_projection_plausibility_caps
import pandas as pd
import numpy as np
import os, datetime
...
_VALUE_ANCHORS = [0.75, 2.00, 5.00, 10.0, 15.0, 20.0, 28.0, 36.0, 42.0, 50.0, 56.0, 62.0]
...
def make_decision(state: WRAgentState):
    ...
    if team_nm and roster:
        val_dec, rep_note = decision_fair_aav_with_replacement(
            val, grade_to_market_value, cg, roster, "WR",
        )
