...
from backend.agent.stat_projection_utils import coverage_target_load_17, inactivity_retirement_penalty, apply_inactivity_to_projection_list, apply_projection_plausibility_caps
import pandas as pd
import numpy as np
import os, datetime
...
def make_decision(state: SAgentState):
    ...
    if team_nm and roster:
        val_dec, rep_note = decision_fair_aav_with_replacement(
            val, grade_to_market_value, cg, roster, "S",
        )
