...
async def team_roster(team: str = Query(..., description="Team name"), analysis_year: int = Query(2025)):
    ...
    need_score, need_label = compute_positional_need(
        roster, position_df=df_players, team=team, reference_year=analysis_year, position_key="DI",
    )
    ...
    if req.team:
        ...
        if re_signing:
            ...
            need_score, need_label = compute_positional_need(
                ...
                exclude_player=req.player_name, reference_year=analysis_year, position_key="DI",
            )
            player_cap = next(
            ...
            need_score, need_label = compute_positional_need(
                roster, position_df=df_players, team=req.team, reference_year=analysis_year, position_key="DI",
            )
            player_cap = 0.0
