...
async def team_roster(team: str = Query(...), analysis_year: int = Query(2025)):
    if df_players.empty:
        raise HTTPException(status_code=503, detail="Player database not loaded.")
    roster = get_team_roster(team, df_players, grade_col=C_GRADE_COL, snap_col=C_SNAP_COL, reference_year=analysis_year)
    need_score, need_label = compute_positional_need(roster, position_df=df_players, team=team,
                                                      ...
                                                      prod_stat_cols=C_PROD_STATS, reference_year=analysis_year,
                                                      position_key="C")
...
async def evaluate_player(req: EvaluationRequest):
    ...
    if req.team:
        roster = get_team_roster(req.team, df_players, grade_col=C_GRADE_COL, snap_col=C_SNAP_COL, reference_year=analysis_year)
        ...
        if re_signing:
            ...
            need_score, need_label = compute_positional_need(roster_without, position_df=df_players,
                ...
                snap_col=C_SNAP_COL, prod_stat_cols=C_PROD_STATS, reference_year=analysis_year,
                position_key="C")
            ...
            need_score, need_label = compute_positional_need(roster, position_df=df_players,
                team=req.team, grade_col=C_GRADE_COL, snap_col=C_SNAP_COL, prod_stat_cols=C_PROD_STATS, reference_year=analysis_year,
                position_key="C")
            player_cap = 0.0
