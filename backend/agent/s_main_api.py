"""S (Safety) GM Agent API — port 8013"""
import asyncio
...
from backend.agent.s_agent_graph import s_gm_agent, S_CSV_PATH
...
app = FastAPI(title="NFL S GM Agent API")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

S_GRADE_COL  = "grades_coverage"
S_SNAP_COL   = "snap_counts_defense"
S_PROD_STATS = ["tackles", "interceptions", "pass_break_ups"]

df_players = pd.DataFrame()
if os.path.exists(S_CSV_PATH):
    df_players = pd.read_csv(S_CSV_PATH)
    print(f"[S API] Loaded {len(df_players)} rows")


@app.get("/s-players")
async def get_s_players():
    if df_players.empty:
...
async def team_roster(team: str = Query(...), analysis_year: int = Query(2025)):
    if df_players.empty:
        raise HTTPException(status_code=503, detail="Player database not loaded.")
    roster = get_team_roster(team, df_players, grade_col=S_GRADE_COL, snap_col=S_SNAP_COL, reference_year=analysis_year)
    need_score, need_label = compute_positional_need(roster, position_df=df_players, team=team,
                                                      grade_col=S_GRADE_COL, snap_col=S_SNAP_COL,
                                                      prod_stat_cols=S_PROD_STATS, reference_year=analysis_year,
                                                      position_key="S")
...
async def evaluate_player(req: EvaluationRequest):
    ...
    if req.team:
        roster = get_team_roster(req.team, df_players, grade_col=S_GRADE_COL, snap_col=S_SNAP_COL, reference_year=analysis_year)
        ...
        if re_signing:
            ...
            need_score, need_label = compute_positional_need(roster_without, position_df=df_players,
                team=req.team, exclude_player=req.player_name, grade_col=S_GRADE_COL,
                snap_col=S_SNAP_COL, prod_stat_cols=S_PROD_STATS, reference_year=analysis_year)
            ...
            need_score, need_label = compute_positional_need(roster, position_df=df_players,
                team=req.team, grade_col=S_GRADE_COL, snap_col=S_SNAP_COL, prod_stat_cols=S_PROD_STATS, reference_year=analysis_year,
                position_key="S")
            player_cap = 0.0
    ...
    final_state = await loop.run_in_executor(_thread_pool, s_gm_agent.invoke, initial_state)
    if req.team:
