...
def _composite_grade(model_grade, stats_gr):
    return round(0.44 * model_grade + 0.56 * stats_gr, 2)
...
def project_stats(
    ...
    for yr in range(1, contract_years + 1):
        ...
        trend_mult = projection_trend_multiplier("LB", age, yr, player_yoy)
...
def compute_contract_value(
    ...
    for yr in range(1, contract_years + 1):
        ...
        base_value = fair_market_aav_millions(grade, "LB", analysis_year) * snap_rel
...
def predict_performance(state: LBAgentState):
    ...
    raw_mg = _safe_float(history.sort_values("Year").iloc[-1].get("grades_defense"), 60.0)
...
def make_decision(state: LBAgentState):
    ...
    if team_nm and roster:
        ...
        val_dec, rep_note = decision_fair_aav_with_replacement(
            val, lambda g: fair_market_aav_millions(g, "LB", _yr), cg, roster, "LB",
        )
