...
from backend.agent.wr_agent_graph import wr_gm_agent, WR_CSV_PATH
...
app = FastAPI(title="NFL WR GM Agent API")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

WR_GRADE_COL = "grades_offense"
WR_SNAP_COL  = "routes"
WR_PROD_STATS = ["yards", "touchdowns", "targets"]

df_players = pd.DataFrame()
if os.path.exists(WR_CSV_PATH):
    df_players = pd.read_csv(WR_CSV_PATH)
    print(f"[WR API] Loaded {len(df_players)} rows")


@app.get("/wr-players")
async def get_wr_players():
    if df_players.empty:
...
async def team_roster(team: str = Query(...), analysis_year: int = Query(2025)):
    if df_players.empty:
        raise HTTPException(status_code=503, detail="Player database not loaded.")
    roster = get_team_roster(team, df_players, grade_col=WR_GRADE_COL, snap_col=WR_SNAP_COL, reference_year=analysis_year)
    need_score, need_label = compute_positional_need(roster, position_df=df_players, team=team,
                                                      grade_col=WR_GRADE_COL, snap_col=WR_SNAP_COL,
                                                      prod_stat_cols=WR_PROD_STATS, reference_year=analysis_year,
                                                      position_key="WR")
...
async def evaluate_player(req: EvaluationRequest):
    ...
    if req.team:
        roster = get_team_roster(req.team, df_players, grade_col=WR_GRADE_COL, snap_col=WR_SNAP_COL, reference_year=analysis_year)
        ...
        if re_signing:
            ...
            need_score, need_label = compute_positional_need(roster_without, position_df=df_players,
                team=req.team, exclude_player=req.player_name, grade_col=WR_GRADE_COL,
                snap_col=WR_SNAP_COL, prod_stat_cols=WR_PROD_STATS, reference_year=analysis_year,
                position_key="WR")
            ...
            need_score, need_label = compute_positional_need(roster, position_df=df_players,
                team=req.team, grade_col=WR_GRADE_COL, snap_col=WR_SNAP_COL, prod_stat_cols=WR_PROD_STATS, reference_year=analysis_year,
                position_key="WR")
            player_cap = 0.0
    ...
    final_state = await loop.run_in_executor(_thread_pool, wr_gm_agent.invoke, initial_state)
    if req.team:
