...
from backend.agent.cb_agent_graph import cb_gm_agent, CB_CSV_PATH
...
app = FastAPI(title="NFL CB GM Agent API")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

CB_GRADE_COL  = "grades_coverage"
CB_SNAP_COL   = "snap_counts_defense"
CB_PROD_STATS = ["interceptions", "pass_break_ups", "tackles"]

df_players = pd.DataFrame()
if os.path.exists(CB_CSV_PATH):
    df_players = pd.read_csv(CB_CSV_PATH)
    print(f"[CB API] Loaded {len(df_players)} rows")


@app.get("/cb-players")
async def get_cb_players():
    if df_players.empty:
...
async def team_roster(team: str = Query(...), analysis_year: int = Query(2025)):
    if df_players.empty:
        raise HTTPException(status_code=503, detail="Player database not loaded.")
    roster = get_team_roster(team, df_players, grade_col=CB_GRADE_COL, snap_col=CB_SNAP_COL, reference_year=analysis_year)
    need_score, need_label = compute_positional_need(roster, position_df=df_players, team=team,
                                                      grade_col=CB_GRADE_COL, snap_col=CB_SNAP_COL,
                                                      prod_stat_cols=CB_PROD_STATS, reference_year=analysis_year,
                                                      position_key="CB")
...
async def evaluate_player(req: EvaluationRequest):
    ...
    if req.team:
        roster = get_team_roster(req.team, df_players, grade_col=CB_GRADE_COL, snap_col=CB_SNAP_COL, reference_year=analysis_year)
        ...
        if re_signing:
            ...
            need_score, need_label = compute_positional_need(roster_without, position_df=df_players,
                team=req.team, exclude_player=req.player_name, grade_col=CB_GRADE_COL,
                snap_col=CB_SNAP_COL, prod_stat_cols=CB_PROD_STATS, reference_year=analysis_year,
                position_key="CB")
            ...
            need_score, need_label = compute_positional_need(roster, position_df=df_players,
                team=req.team, grade_col=CB_GRADE_COL, snap_col=CB_SNAP_COL, prod_stat_cols=CB_PROD_STATS, reference_year=analysis_year,
                position_key="CB")
            player_cap = 0.0
    ...
    initial_state = {
        ...
        "predicted_tier": "", "projected_tier": "", "confidence": {}, "current_age": 26,
    ...
    final_state = await loop.run_in_executor(_thread_pool, cb_gm_agent.invoke, initial_state)
    if req.team:
