...
def project_stats(
    ...
    for yr in range(1, contract_years + 1):
        ...
        trend_mult = projection_trend_multiplier("CB", age, yr, player_yoy)
...
def compute_contract_value(
    ...
    for yr in range(1, contract_years + 1):
        ...
        base_value = fair_market_aav_millions(grade, "CB", analysis_year) * snap_rel
...
def make_decision(state: CBAgentState):
    ...
    if team_nm and roster:
        ...
        val_dec, rep_note = decision_fair_aav_with_replacement(
            val, lambda g: fair_market_aav_millions(g, "CB", _yr), cg, roster, "CB",
        )
