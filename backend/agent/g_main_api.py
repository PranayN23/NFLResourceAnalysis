"""G (Guard) GM Agent API — port 8009"""
import asyncio
...
from backend.agent.ol_agent_graph import ol_gm_agent, G_CSV_PATH
...
app = FastAPI(title="NFL G GM Agent API")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

G_GRADE_COL  = "grades_pass_block"
G_SNAP_COL   = "snap_counts_offense"
G_PROD_STATS = ["snap_counts_offense", "penalties"]

df_players = pd.DataFrame()
if os.path.exists(G_CSV_PATH):
    df_players = pd.read_csv(G_CSV_PATH)
    print(f"[G API] Loaded {len(df_players)} rows")


@app.get("/g-players")
async def get_g_players():
    if df_players.empty:
...
async def team_roster(team: str = Query(...), analysis_year: int = Query(2025)):
    if df_players.empty:
        raise HTTPException(status_code=503, detail="Player database not loaded.")
    roster = get_team_roster(team, df_players, grade_col=G_GRADE_COL, snap_col=G_SNAP_COL, reference_year=analysis_year)
    need_score, need_label = compute_positional_need(roster, position_df=df_players, team=team,
                                                      grade_col=G_GRADE_COL, snap_col=G_SNAP_COL,
                                                      prod_stat_cols=G_PROD_STATS, reference_year=analysis_year,
                                                      position_key="G")
...
async def evaluate_player(req: EvaluationRequest):
    ...
    if req.team:
        roster = get_team_roster(req.team, df_players, grade_col=G_GRADE_COL, snap_col=G_SNAP_COL, reference_year=analysis_year)
        ...
        if re_signing:
            ...
            need_score, need_label = compute_positional_need(roster_without, position_df=df_players,
                team=req.team, exclude_player=req.player_name, grade_col=G_GRADE_COL,
                snap_col=G_SNAP_COL, prod_stat_cols=G_PROD_STATS, reference_year=analysis_year,
                position_key="G")
            ...
            need_score, need_label = compute_positional_need(roster, position_df=df_players,
                team=req.team, grade_col=G_GRADE_COL, snap_col=G_SNAP_COL, prod_stat_cols=G_PROD_STATS, reference_year=analysis_year,
                position_key="G")
            player_cap = 0.0
