...
from backend.agent.agent_graph import qb_gm_agent, QB_CSV_PATH
...
app = FastAPI(title="NFL QB GM Agent API")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

QB_GRADE_COL = "grades_pass"
QB_SNAP_COL  = "passing_snaps"
QB_PROD_STATS = ["yards", "touchdowns", "dropbacks"]

df_players = pd.DataFrame()
if os.path.exists(QB_CSV_PATH):
    df_players = pd.read_csv(QB_CSV_PATH)
    if "player" not in df_players.columns:
        for candidate in ("Player", "Name", "name"):
            if candidate in df_players.columns:
                ...
                break
    print(f"[QB API] Loaded {len(df_players)} rows")
...
async def team_roster(team: str = Query(...), analysis_year: int = Query(2025)):
    if df_players.empty:
        raise HTTPException(status_code=503, detail="Player database not loaded.")
    roster = get_team_roster(team, df_players, grade_col=QB_GRADE_COL, snap_col=QB_SNAP_COL, reference_year=analysis_year)
    need_score, need_label = compute_positional_need(roster, position_df=df_players, team=team,
                                                      grade_col=QB_GRADE_COL, snap_col=QB_SNAP_COL,
                                                      prod_stat_cols=QB_PROD_STATS, reference_year=analysis_year,
                                                      position_key="QB")
...
async def evaluate_player(req: EvaluationRequest):
    ...
    if req.team:
        ...
        if re_signing:
            ...
            need_score, need_label = compute_positional_need(roster_without, position_df=df_players,
                team=req.team, exclude_player=req.player_name, grade_col=QB_GRADE_COL,
                snap_col=QB_SNAP_COL, prod_stat_cols=QB_PROD_STATS, reference_year=analysis_year,
                position_key="QB")
            ...
            need_score, need_label = compute_positional_need(roster, position_df=df_players,
                team=req.team, grade_col=QB_GRADE_COL, snap_col=QB_SNAP_COL, prod_stat_cols=QB_PROD_STATS, reference_year=analysis_year,
                position_key="QB")
            player_cap = 0.0
    ...
    if req.is_extension and team_ctx:
        # Extension starts at effective_year — current cap is irrelevant; assume space available
    ...
    initial_state = {
        ...
        "decision": "", "reasoning": "",
        "projected_signing_role": "", "projected_signing_role_reason": "",
        **team_state_fields,
    }

    loop = asyncio.get_event_loop()
    final_state = await loop.run_in_executor(_thread_pool, qb_gm_agent.invoke, initial_state)
    ...
    response = {
        ...
        "data": {
            ...
            "projected_signing_role":        final_state.get("projected_signing_role", ""),
            "projected_signing_role_reason": final_state.get("projected_signing_role_reason", ""),
        },
    }
    if team_ctx:
