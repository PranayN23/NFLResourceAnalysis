...
from backend.agent.lb_agent_graph import lb_gm_agent, CSV_PATH
import pandas as pd
import uvicorn
import os

app = FastAPI(title="NFL GM Agent API - Linebackers")
    ...
    if 'player' not in df_players.columns:
        ...
        for p in possible:
            if p in df_players.columns:
                ...
                break
    print(f"Loaded LB Player Database: {len(df_players)} rows")
else:
    print("WARNING: LB Player CSV not found. Agent will fail to retrieve history.")
...
@app.post("/evaluate-lb")
async def evaluate_player(req: EvaluationRequest):
    """
    Evaluates a Free Agent LB by looking up their stats and running the GM Agent Workflow.
    """
    ...
    # 3. Run Graph
    final_state = lb_gm_agent.invoke(initial_state)
