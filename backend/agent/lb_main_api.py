...
async def team_roster(team: str = Query(...)):
    if df_players.empty:
        raise HTTPException(status_code=503, detail="Player database not loaded.")
    roster = get_team_roster(team, df_players, grade_col=LB_GRADE_COL, snap_col=LB_SNAP_COL)
...
async def evaluate_player(req: EvaluationRequest):
    ...
    if req.team:
        roster = get_team_roster(req.team, df_players, grade_col=LB_GRADE_COL, snap_col=LB_SNAP_COL)
        ...
        if re_signing:
            ...
            need_score, need_label = compute_positional_need(roster_without, position_df=df_players,
                ...
                snap_col=LB_SNAP_COL, prod_stat_cols=LB_PROD_STATS)
            ...
            need_score, need_label = compute_positional_need(roster, position_df=df_players,
                team=req.team, grade_col=LB_GRADE_COL, snap_col=LB_SNAP_COL, prod_stat_cols=LB_PROD_STATS)
            player_cap = 0.0
