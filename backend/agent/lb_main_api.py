...
from backend.agent.lb_agent_graph import lb_gm_agent, LB_CSV_PATH
...
app = FastAPI(title="NFL LB GM Agent API")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

LB_GRADE_COL  = "grades_defense"
LB_SNAP_COL   = "snap_counts_defense"
LB_PROD_STATS = ["tackles", "assists", "forced_fumbles"]

df_players = pd.DataFrame()
if os.path.exists(LB_CSV_PATH):
    df_players = pd.read_csv(LB_CSV_PATH)
    print(f"[LB API] Loaded {len(df_players)} rows")


@app.get("/lb-players")
async def get_lb_players():
    if df_players.empty:
...
async def team_roster(team: str = Query(...), analysis_year: int = Query(2025)):
    if df_players.empty:
        raise HTTPException(status_code=503, detail="Player database not loaded.")
    roster = get_team_roster(team, df_players, grade_col=LB_GRADE_COL, snap_col=LB_SNAP_COL, reference_year=analysis_year)
    need_score, need_label = compute_positional_need(roster, position_df=df_players, team=team,
                                                      grade_col=LB_GRADE_COL, snap_col=LB_SNAP_COL,
                                                      prod_stat_cols=LB_PROD_STATS, reference_year=analysis_year,
                                                      position_key="LB")
...
async def evaluate_player(req: EvaluationRequest):
    ...
    if req.team:
        roster = get_team_roster(req.team, df_players, grade_col=LB_GRADE_COL, snap_col=LB_SNAP_COL, reference_year=analysis_year)
        ...
        if re_signing:
            ...
            need_score, need_label = compute_positional_need(roster_without, position_df=df_players,
                team=req.team, exclude_player=req.player_name, grade_col=LB_GRADE_COL,
                snap_col=LB_SNAP_COL, prod_stat_cols=LB_PROD_STATS, reference_year=analysis_year,
                position_key="LB")
            ...
            need_score, need_label = compute_positional_need(roster, position_df=df_players,
                team=req.team, grade_col=LB_GRADE_COL, snap_col=LB_SNAP_COL, prod_stat_cols=LB_PROD_STATS, reference_year=analysis_year,
                position_key="LB")
            player_cap = 0.0
    ...
    initial_state = {
        ...
        "predicted_tier": "", "projected_tier": "", "confidence": {}, "current_age": 27,
    ...
    final_state = await loop.run_in_executor(_thread_pool, lb_gm_agent.invoke, initial_state)
    if req.team:
