...
# CONFIG
# ==========================================
DATA_FILE     = os.path.join(os.path.dirname(__file__), "../DI.csv")
MODEL_OUT     = os.path.join(os.path.dirname(__file__), "di_best_classifier.pth")
...
# when training Player_Model_DI.py — order matters for the scaler.
#
...
def prepare_data():
    ...
    numeric_cols = [
        ...
        "snap_counts_dl_over_t", "snap_counts_dl_outside_t", "Cap_Space",
    ]
    ...
    # --------------------------------------------------
    # Feature engineering — mirrors Player_Model_DI.py exactly
    # --------------------------------------------------
    def safe_div(a, b):
    ...
    for col in ["grades_defense", "grades_pass_rush_defense", "grades_run_defense",
                "pressure_rate", "sack_rate", "stop_rate", "a_gap_share", "b_gap_share",
                "team_performance_proxy"]:
...
if MODE == "VALIDATION":
    ...
    engine  = DIModelInference(MODEL_OUT, scaler_path=SCALER_OUT, xgb_path=XGB_MODEL_OUT)
    results = []
    ...
    out_path = os.path.join(os.path.dirname(__file__), "DI_2024_Validation_Results.csv")
...
# ==========================================
    ...
    final_out = os.path.join(os.path.dirname(__file__), "DI_2025_Final_Rankings.csv")
