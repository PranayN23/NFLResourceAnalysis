...
# 1. SETUP & CONFIG
# ==========================================
DATA_FILE = os.path.join(os.path.dirname(__file__), "../CB.csv")
...
print(f"==== STARTING CB TIME2VEC TRANSFORMER PREDICTIONS (Mode: {MODE}) ====")
...
engine = CBModelInference(MODEL_OUT, scaler_path=SCALER_OUT)
...
if MODE == "VALIDATION":
    ...
    final_out = os.path.join(os.path.dirname(__file__), "CB_2025_Final_Rankings.csv")
    final_2025.to_csv(final_out, index=False)

    print("\n" + "="*80)
    print("TOP 15 CB RANKINGS FOR 2025 (TIME2VEC TRANSFORMER)")
    print("="*80)
