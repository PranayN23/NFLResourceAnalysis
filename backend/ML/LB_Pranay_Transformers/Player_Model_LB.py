...
def create_sequences_temporal(df_scaled, df_raw, seq_len, features, target_col):
    sequences, targets, masks, years, players = [], [], [], [], []
    ...
    for player in df_raw["player"].unique():
        ...
        for i in range(len(p_raw)):
            ...
            players.append(df_raw.iloc[i]["player"])
...
if __name__ == "__main__":
    ...
    players_val = players_all[val_idx]
    ...
    # --------------------------------------------------
    # Naive career-mean baseline (proper alignment)
    # --------------------------------------------------

    career_means = (
        df_clean[df_clean["Year"] <= TRAIN_END_YEAR]
        .groupby("player")["grades_defense"]
        .mean()
    )

    global_mean = df_clean[
        df_clean["Year"] <= TRAIN_END_YEAR
    ]["grades_defense"].mean()

    players_test = players_all[test_idx]

    baseline_preds = np.array([
        career_means.get(p, global_mean)
