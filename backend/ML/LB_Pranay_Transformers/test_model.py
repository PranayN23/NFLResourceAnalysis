...
def run_test():
    parser = argparse.ArgumentParser(description="Test LB Prediction Models")
    parser.add_argument(
        "--mode", type=str, default="transformer",
        choices=["transformer", "xgb", "ensemble"],
        help="Model component to evaluate (default: transformer)"
    )
    ...
    parser.add_argument(
        ...
        help="Number of top LB players by snap count to evaluate per year (default: 40)"
    )
    parser.add_argument("--export", action="store_true", help="Export per-player results to CSV")
    args = parser.parse_args()

    base_dir = os.path.dirname(__file__)
    TRANSFORMER_PATH = os.path.join(base_dir, "lb_best_classifier.pth")
    XGB_PATH         = os.path.join(base_dir, "lb_best_xgb.joblib")
    SCALER_PATH      = os.path.join(base_dir, "lb_player_scaler.joblib")
    ...
    if not os.path.exists(TRANSFORMER_PATH) and args.mode != "xgb":
        ...
        print("Run Player_Model_LB.py first to generate the checkpoint.")
        return
    if not os.path.exists(CSV_PATH):
        print(f"Error: Data file not found at {CSV_PATH}")
        return

    engine = LBModelInference(
        TRANSFORMER_PATH,
    ...
    numeric_cols = [
        ...
        "grades_coverage_defense", "grades_run_defense",
        "grades_pass_rush_defense", "total_pressures", "sacks",
        "targets", "interceptions", "pass_break_ups",
    ]
    ...
    print(f"\n[TESTING LB MODE: {args.mode.upper()}]")
    ...
    for year in range(args.year_start, args.year_end + 1):
        ...
        print(f"\n{'='*110}")
        print(f"  PREDICTING {year} LB PERFORMANCE  (mode: {args.mode})")
        print(f"{'='*110}")
        print(
    ...
    if args.export and export_rows:
        ...
        export_path = os.path.join(
            base_dir,
            f"LB_Test_Results_{args.mode}_{args.year_start}_{args.year_end}.csv"
        )
    ...
    # --------------------------------------------------
    # LB-specific insights
    # --------------------------------------------------
    if not export_rows:
        print("\n(Run with --export for LB-specific breakdown)")
        return
    ...
    # LB age brackets — similar to ED, coverage athleticism declines mid-career
    print("\n  Tier Accuracy by Age Group:")
    age_groups = [((0,  25), "Young   (≤25)"),
                  ((26, 28), "Prime   (26-28)"),
                  ((29, 31), "Senior  (29-31)"),
                  ((32, 99), "Veteran (32+)")]
