...
def load_sequences(filepath, seq_len=4):
    ...
    cols_to_convert = [c for c in df.columns if c not in metadata_cols]
    ...
    # WR Specifc: Normalize by 1000 since CSV has raw Grade*Snaps
    ...
    if "grades_offense" in df.columns:
        df["efficiency"] = df["grades_offense"] / 100.0
    else: 
        df["efficiency"] = 0
        
    if "total_snaps" in df.columns:
    ...
    for pid, group in groups:
        ...
        for i in range(1, len(group)):
            ...
            meta.append({
                "player_id": group.iloc[i]["player_id"],
                "player": group.iloc[i]["player"], 
...
if __name__ == "__main__":
    print("Loading WR Sequence Data...")
    SEQ_LEN = 4 
    X, T, y, meta_df, feats = load_sequences("backend/ML/WR.csv", seq_len=SEQ_LEN)
    ...
    y_test = torch.tensor(y[test_mask], dtype=torch.float32).unsqueeze(1) 
    ...
    model = WRTransformer(num_features=F, time_emb_dim=16, d_model=64, nhead=4, num_layers=3).to(device)
