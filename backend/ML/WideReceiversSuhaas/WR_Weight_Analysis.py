...
def run_weight_analysis(validation_year):
    ...
    # 3. ENSEMBLE
    # ==========================
    trans_results = meta_df[test_mask].reset_index(drop=True)
    trans_results["Pred_Trans"] = trans_preds
    
    xgb_results = test_xgb.reset_index(drop=True) 
    # Use reset_index on both to align by common keys? 
    # Better to join on keys.
    xgb_results["Pred_XGB"] = xgb_preds
    
    # Join on Player + Team since ID might be missing in test_xgb? 
    # Actually load_xgb_data returns player_id.
    ensemble_df = pd.merge(xgb_results, trans_results[["player_id", "Pred_Trans"]], on="player_id", how="inner")
