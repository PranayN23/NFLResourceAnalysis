...
def analyze():
    ...
    XGB_PATH = os.path.join(base_dir, "best_lb_xgb.joblib")
    SCALER_PATH = os.path.join(base_dir, "lb_scaler.joblib")
    CSV_PATH = os.path.join(base_dir, "../LB.csv")

    engine = LBModelInference(TRANSFORMER_PATH, scaler_path=SCALER_PATH, xgb_path=XGB_PATH)
    ...
    print("Running Year-by-Year Multi-Factor Analysis for LBs...")
    for year in years:
        df_year = df[(df['Year'] == year) & (df['snap_counts_defense'] >= 200)]
        y_true_tiers = []
        y_pred_tiers = []
        errors = []
        ...
        for _, row in df_year.iterrows():
            ...
            if pred_tier != "No Data":
                ...
                prev_year_grade = history.iloc[-1]['grades_defense'] if not history.empty else 0.0
