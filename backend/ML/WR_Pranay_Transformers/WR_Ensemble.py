...
from backend.ML.WR_Pranay_Transformers.Player_Model_WR import (
...
# CONFIG
# ==========================================
DATA_FILE     = os.path.join(os.path.dirname(__file__), "../WR.csv")
MODEL_OUT     = os.path.join(os.path.dirname(__file__), "wr_best_transformer.pth")
SCALER_OUT    = os.path.join(os.path.dirname(__file__), "wr_player_scaler.joblib")
XGB_MODEL_OUT = os.path.join(os.path.dirname(__file__), "wr_best_xgb.joblib")
...
def prepare_data():
    ...
    numeric_cols = [
        "grades_offense", "grades_pass_route", "grades_hands_drop", "grades_hands_fumble",
    ...
    for col in ["grades_offense", "grades_pass_route", "yprr", "avg_depth_of_target",
                "caught_percent", "drop_rate", "yards", "targets", "touchdowns",
                "yards_after_catch", "targeted_qb_rating", "slot_share", "wide_share",
                "team_performance_proxy"]:
...
if MODE == "VALIDATION":
    ...
    engine  = WRModelInference(MODEL_OUT, scaler_path=SCALER_OUT, xgb_path=XGB_MODEL_OUT)
    results = []
    ...
    out_path = os.path.join(os.path.dirname(__file__), "WR_2024_Validation_Results.csv")
...
# ==========================================
    ...
    final_out = os.path.join(os.path.dirname(__file__), "WR_2025_Final_Rankings.csv")
    final_2025.to_csv(final_out, index=False)

    print("\nTop 15 WR for 2025:")
    print(final_2025.head(15).to_string(index=False))

print("\n=== WR MODEL PIPELINE COMPLETE ===")
