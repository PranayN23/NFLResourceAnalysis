...
def run_test():
    parser = argparse.ArgumentParser(description="Test ED Prediction Models")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Number of top ED players by snap count to evaluate per year (default: 40)"
    )
    parser.add_argument("--export", action="store_true", help="Export per-player results to CSV")
    args = parser.parse_args()

    base_dir = os.path.dirname(__file__)
    TRANSFORMER_PATH = os.path.join(base_dir, "../ED_Transformers/ed_best_classifier.pth")
    XGB_PATH         = os.path.join(base_dir, "../ED_Transformers/ed_best_xgb.joblib")
    SCALER_PATH      = os.path.join(base_dir, "../ED_Transformers/ed_player_scaler.joblib")
    CSV_PATH         = os.path.join(base_dir, "../ED.csv")
    ...
    if not os.path.exists(TRANSFORMER_PATH) and args.mode != "xgb":
        ...
        print("Run Player_Model_ED.py first to generate the checkpoint.")
        return
    ...
    print(f"\n[TESTING ED MODE: {args.mode.upper()}]")
    ...
    for year in range(args.year_start, args.year_end + 1):
        ...
        print(f"\n{'='*110}")
        print(f"  PREDICTING {year} ED PERFORMANCE  (mode: {args.mode})")
        print(f"{'='*110}")
        print(
    ...
    if args.export and export_rows:
        ...
        export_path = os.path.join(
            base_dir,
            f"ED_Test_Results_{args.mode}_{args.year_start}_{args.year_end}.csv"
        )
    ...
    # --------------------------------------------------
    # ED-specific insights (requires --export to have row data)
    # --------------------------------------------------
    if not export_rows:
        print("\n(Run with --export for ED-specific breakdown)")
        return
    ...
    # Accuracy by age group (ED-specific brackets — edge rushers peak earlier)
