...
if MODE == "VALIDATION":
    ...
    X_val_t = torch.FloatTensor(X_scaled[test_mask])
    T_val_t = torch.FloatTensor(T[test_mask])
    
    with torch.no_grad():
        val_preds = transformer(X_val_t, T_val_t).flatten().numpy()
    ...
    for pid in players_2024: # We calculate for all active 
        ...
        with torch.no_grad():
            t_in = torch.FloatTensor(scaled_seq).unsqueeze(0)
            time_in = torch.FloatTensor(times).unsqueeze(0)
            pred = transformer(t_in, time_in).item()
...
# WEIGHTS (Shifting more to Transformer for "Upside")
W_XGB = 0.50
W_TRANS = 0.50
...
if MODE == "VALIDATION":
    ...
    cols = ["player", "Team", "Pred_XGB", "Pred_Transformer", "TE_Value_Score_2025"]
